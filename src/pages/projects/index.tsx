...
export const ProjectsPage = () => {
  ...
  const [showDeleteModal, setShowDeleteModal] = useState<false | ProjectShort>(false);
  ...
  const onFetchData = async () => {
      ...
      if (result.isOk) {
        dispatch(setProjects((result.body.projects as any[]).reverse()));
      }
      if (result.isUnauth) {
    ...
    <>
      <div className="projects-container">
        <div className="project-content-container">
          <Typography type="h1" text={dictionary.projects.label} color={colors.black} />
          {!projects.length ? (
            <div
              style={{
                display: 'flex',
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                <table style={{ marginTop: 30 }}>
                  <tr>
                    <th>
                      <Typography color={colors.gray30} text={dictionary.projects.title} />
                    </th>
                    <th>
                      <Typography color={colors.gray30} text={dictionary.projects.last_modified} />
                    </th>
                    <th></th>
                  </tr>
                  {projects.map((p) => (
                    <tr
                      onClick={() => onRowClick(p.projectId!)}
                      key={`${p.projectId}-${p.title}`}
                    >
                      <td style={{height: 63, width: '33%'}}>
                        <ProjectTitle project={p} onSuccessRename={onFetchData} />
                      </td>
                      <td>
                        <Typography
                          color={colors.gray30}
                      ...
                      <td>
                        <div
                          onClick={(e) => onClickDeleteProject(e, p)}
                          className="delete-project-container"
                        >
                          <div className="delete-icon">
                            <PlusIcon />
                          </div>
                          <Typography color={colors.black} text={dictionary.delete} />
                          <div style={{ width: 10 }} />
                        </div>
                      </td>
                    </tr>
      ...
      <Modal onClose={() => setShowAddModal(false)} showModal={showAddModal}>
