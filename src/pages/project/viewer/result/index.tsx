...
import {
  useCompiledSuccesInfo, useCurrentProject,
  useUser
...
export const Result = () => {
  ...
  const onPress = () => {
    ...
    setTimeout(() => {
      const print = async () => {
        const mathElements = document.querySelectorAll("#compile-result"); 
        await (window as any).MathJax.typesetPromise(mathElements);
        reactToPrintFn();
      }
      print();
