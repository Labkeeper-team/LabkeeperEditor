...
export const MAX_LINE_LENGTH_BEFORE_SETTING_INFL_TO_NEW_LINE =
    (MAX_LINE_LENGTH * 5) / 4;
...
export function renderArrayWithoutInflToLatex(array) {
    ...
    const addStringPresentation = (str) => {
        ...
        if (
            currentLineLength + str.length > MAX_LINE_LENGTH &&
            currentLineLength > 0
        ) {
            arrayPresentation += '\\\\ ';
            currentLineLength = 0;
    ...
    clearFormulaItems.forEach((formula, index) => {
        const comma =
            index + 1 < clearFormulaItems.length && clearFormulaItems.length > 1
                ? ',\\ '
                : '';
...
export function renderArrayWithInflToLatex(array, initialGap) {
    ...
    const singleInflString = hasSingleInflValue
        ? parser.parse(array[0].infl)
        : '';
    const displaySingleInflOnSameLine =
        maxLineLength < MAX_LINE_LENGTH_BEFORE_SETTING_INFL_TO_NEW_LINE &&
        singleInflString.length < MAX_INFL_LENGTH_TO_DISPLAY_ON_SAME_LINE;
    const resetValuesToNewLine =
        initialGap > VAR_MAX_LENGTH_BEFORE_RESETING_TO_NEW_LINE;
    const displaySingleInfl =
        hasSingleInflValue && parseFloat(parser.parse(array[0].infl));
...
export function renderArrayOfValuedFormulasToLatex(
    ...
    const formulasCutted = isCutted
        ? [formulas[0], formulas[1], formulas[2], formulas[3], formulas[4]]
        : formulas;
    ...
    const addStringPresentation = (str, asciiStringLength) => {
        if (
            currentLineLength + asciiStringLength > MAX_LINE_LENGTH &&
            currentLineLength > 0
        ) {
            arrayPresentation += '\\\\ ';
            currentLineLength = 0;
    ...
    formulasCutted.forEach((formula, index) => {
        let parsedFormula = parser.parse(
            customizeSpecialSymbolsBeforeParsing(formula)
        );
        if (maySplit) {
...
export function renderVariableToLatex(name) {
    ...
    const tryParseToText = (v) =>
        isAlphabetic(v)
            ? `\\${v}`
            : v.length > MAX_LENGTH_VAR
...
export function renderFormulaToLatex(str, variables, maySplitBySqrt = false) {
    ...
    variables.forEach((v) => {
        ...
        const sigma = `sigma(${v})`;
        aliased_equation = aliased_equation.replaceAll(
            sigma,
            `\\text{${toAlias(sigma)}}`
        );
        if (v !== presentation) {
    ...
    variables.forEach((v) => {
        ...
        parsed_equation = parsed_equation.replaceAll(
            `\\text{${toAlias(sigma)}}`,
            `\\sigma(${presentation})`
        );
        if (v !== presentation) {
...
function tryToSplitFormulaByPlusesInSqrt(formula) {
    ...
    const formulaWithoutSqrt = formula.substring(
        '\\sqrt{'.length,
        formula.length - 1
    );
