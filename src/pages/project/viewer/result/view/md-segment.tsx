...
export const MdSegment = forwardRef<HTMLDivElement, any>(
  ({ segment }, ref) => {
    ...
    return (
      <div
        ref={ref ?? segmentRef}
        className={classNames({ 'active-result-block-container': activeIndex, 'markdown-body': true, 'result-segment': true})}
      >
        <Markdown
          remarkPlugins={[remarkBreaks, remarkMath, remarkGfm]}
          components={{
            img(props) {
              return (<img src={props.src} alt={props.alt} width="500px" style={{
                display: 'block',
                marginLeft: 'auto',
                marginRight: 'auto'
              }}/>)
