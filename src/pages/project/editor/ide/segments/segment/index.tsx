...
export const SegmentEditor = memo(
    (props: { segment: Segment; index: number; segmentCount: number }) => {
        ...
        const isActiveSegment = useSelector(
            useIsSegmentIsActive(props.segment.id!)
        );
        ...
        const debounceCompile = useCallback(
            debounce(
                ...
                async (value, projectId) => {
                    ...
                    const tmpProgram: Program = JSON.parse(
                        JSON.stringify(program)
                    );
                    tmpProgram.segments = tmpProgram.segments.map(
                        (item, index_) => {
                            if (index_ === index) {
        ...
        const onBlur = useCallback(async () => {
            ...
            setTimeout(async () => {
                dispatch(
                    changeSegmentText({ text: tempText, index: props.index })
                );
                saveProgramDirectly();
            });
                                    ...
                                    if (res.isOk) {
                                        ...
                                        const cursorPosition =
                                            editor?.current?.view?.state
                                                .selection.main.head; // Получаем позицию курсора
                                        onChange(
        ...
        useEffect(() => {
            ...
            if (segmentTempErrors.length) {
                ...
                sortedErrors.forEach((segmentError) => {
                    ...
                    const row =
                        docTextSpliitedByNewLine[segmentError.payload.line];
                    ...
                    <div className="editor-rules">
                        ...
                        >
                            ...
                            <Checkbox
                                ...
                                onChange={(v) =>
                                    onChangeVisible(v, 'hideGeneralFormula')
                                }
                            ...
                            <Checkbox
                                ...
                                onChange={(v) =>
                                    onChangeVisible(v, 'hideInflAssignment')
                                }
