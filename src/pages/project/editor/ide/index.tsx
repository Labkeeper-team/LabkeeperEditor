...
export const Ide = () => {
  ...
  const isAutocomleteLoading = useSelector((state: StorageState) => state.settings.isAutompleteLoading);
  ...
  const onClickRun = useCallback(async () => {
    if (program) {
      try {
        setIsLoading(true);
        if (user.isAuthenticated && projectId && !isNaN(+projectId) && !isReadonly) {
          const result = await saveProgramRequest(projectId.toString(), program);
  ...
  const buttonTitle = useMemo(() => {
    ...
    return program?.segments.length ? dictionary.run : dictionary.label_add_code;
