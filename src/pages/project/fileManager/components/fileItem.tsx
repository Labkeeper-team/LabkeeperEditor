...
export const FileItem = (props: {file: IFile, onAfterSuccessChange?: () => any, showDropDown: boolean}) => {
    ...
    const onDeleteFile = async () => {
        ...
        const result = await deleteFileRequest(props.file.fileName, project?.projectId?.toString())
        if (result.isUnauth) {
    ...
    const onBlurInput = async () => {
        ...
        const result = await renameFileRequest(props.file.fileName, fileName, project?.projectId?.toString())
        if (result.isUnauth) {
            toast(dictionary.filemanager.errors.sessionExpired, {type: 'error'});
            dispatch(logoutAction)
        }
    ...
    return <div className="file-item ">
            <FileIcon />
            <div className={classNames({'hoverable-item': !editMode})} onClick={!editMode ? () =>  window.open(props.file.url, '_blank') : undefined}>
                <input
                    ref={inputRef as any}
                    value={fileName}
                    onChange={editMode ? (e) => onChange(e.target.value) : undefined}
                    onBlur={editMode ? onBlurInput : undefined}
                    onKeyDown={(e) => e.key === 'Enter' ? onBlurInput() : () => {}}
                    disabled={!editMode}
                    className={`${classNames('change-filename-input', {disabled: !editMode})}`}
                    style={{ pointerEvents: !editMode ? 'none' : 'auto' }} // Добавьте этот стиль
                />
            </div>
            {props.showDropDown ? <DropdownMenu>
            {   showDropdown
                ? <>
                    <div onClick={onDeleteFile} className="delete-file-container">
                        <div className="delete-icon">
                        <PlusIcon />
                        </div>
                        <Typography color={colors.gray10} text={dictionary.delete} />
                    </div>
                    <div onClick={onPressChangeButton} className="edit-file-container">
                        <div className="edit-icon">
                        <PencilIcon />
                        </div>
                        <Typography color={colors.gray10} text={dictionary.filemanager.edit} />
                    </div>
                    </>
                : null}
            </DropdownMenu>: <div style={{width: 28}}/>}
        </div>
}