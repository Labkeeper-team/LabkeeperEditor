...
export const ru: Translations = {
    ...
    tokens_page: {
        ...
        usage_items: [
            {
                heading:
                    'Работа с AI-ассистентом (GPT): 2 токена = 1 текстовый запрос.',
                body:
                    'Попросите нейросеть написать структуру документа, перевести текст в формат Markdown или найти ошибку в коде.',
            },
            {
                heading: 'Генерация изображений: 10 токенов = 1 картинка',
                body:
                    'Создавайте уникальные иллюстрации и графики для ваших статей и лабораторных работ по одному текстовому описанию.',
            },
            {
                heading:
                    'Серверная компиляция (LaTeX): 1 токен = 1 секунда работы',
                body:
                    'Собирайте самые тяжелые PDF-документы без зависаний. Токены списываются строго за время работы мощного удалённого компилятора.',
            },
            {
                heading: 'Гибридный синтаксис',
                body: 'Используйте легковесные вставки Markdown прямо в сложном LaTeX-коде для ускорения работы.',

            },
        ],
        modal: {
            title: 'Покупка токенов',
            gateway_notice: 'Оплата производится через безопасный платёжный шлюз ЮKassa',
