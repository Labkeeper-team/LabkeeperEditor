...
export class RouteSetup {
    ...
    async setupListFilesCustom(
        files: { fileName: string; url: string; autogenerated?: boolean }[]
    ) {
        await this.page.route(
                ...
                await route.fulfill({
                    ...
                    body: JSON.stringify({
                        files: files.map((file) => ({
                            autogenerated: file.autogenerated ?? false,
    ...
    async setupStaticFileContent(urlPath: string, content: string) {
        await this.page.route(
