...
import { FileService } from '../../viewModel/domain/FileService.ts';

const mockGenerateSuffix = jest.spyOn(FileService, 'generateSuffix')
...
Сценарий:
1. Новый файл вставленный через Ctr+V должен быть назван file_seg${segment_id}_${suffix}.png,
 */
...
вставки file_seg${segment_id}_${suffix}.png, file_seg${segment_id}_${suffix}(1).png, ... file_seg${segment_id}_${suffix}(4).png
2. Новый файл вставленный через Ctr+V должен быть назван также, но следующей
версией file_seg${segment_id}_${suffix}(5).png
 */
test('add-new-file-via-CtrV-test_3', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `file_seg5_${suffix}.png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(1).png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(2).png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(3).png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(4).png`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = 5;

    const result = fileService.calculateNumberFile(segment_id, 'vgh87ygvh.png')

    expect(result).toBe(
        `file_seg5_${suffix}(5).png`
    );
});
...
вставки file_seg${segment_id}_${suffix}(i).png, но i это не {0, ... 3}
...
Пример: имеем file_seg${segment_id}_${suffix}.png, file_seg${segment_id}_${suffix}(1).png, file_seg${segment_id}_${suffix}(3).png,
file_seg${segment_id}_${suffix}(7).png
Вывод: file_seg${segment_id}_${suffix}(2).png
 */
test('add-new-file-via-CtrV-test_4', () => {
    ...
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `file_seg5_${suffix}.png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(1).png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(7).png`, url: '25' },
        { autogenerated: true, fileName: `file_seg5_${suffix}(3).png`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = 5;
    ...
    expect(result).toBe(
        `file_seg5_${suffix}(2).png`
    );
});

/*
Тест на добавление файла с уже существующим названием a.b.c.png (c большим кол-вом точек) и первый файл без суффикса
 */
...
/*
Тест на добавление файла с уже существующим названием a.b.c.png (c большим кол-вом точек) и первый файл с таким же суффиксом
 */
test('add-file_with_exist_name-with-dots-from-add-file', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `a.b.c_${suffix}.png`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = null;
    expect(fileService.calculateNumberFile(segment_id, 'a.b.c.png')).toBe(
...
/*
Тест на добавление файла с уже существующим названием (((_${suffix}.png
 */
test('add-file_with_exist_name-with-brackets-from-add-file-with-suffix', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `(((_${suffix}.png`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = null;
    expect(fileService.calculateNumberFile(segment_id, '(((.png')).toBe(
...
test('add_new-file-without-ext-from-add-file', () => {
    const { fileService } = mockContext();
    const segment_id = null;
    expect(fileService.calculateNumberFile(segment_id, 'abc')).toBe(`abc_${suffix}`);
});

/*
Тест на добавление уже существующего файла без расширения "abc" и без суффикса
В результате получаем название с суффиксом, без индекса
 */
test('add-file-with-exist-name-from-add-file-without-ext', () => {
    ...
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'abc')).toBe(`abc_${suffix}`);
});

/*
Тест на добавление уже существующего файла без расширения "abc" и с коллизией суффиксов
В результате получаем название с суффиксом и индексом
 */
test('add-file-with-exist-name-from-add-file-with-suffixes-and-without-ext', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `abc_${suffix}`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'abc')).toBe(`abc_${suffix}(1)`);
});

/*
Тест на добавление уже существующего файла без суффикса
 */
test('add-file-with-exist-name-from-add-file-without-suffix', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: 'a.png', url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'a.png')).toBe(`a_${suffix}.png`);
});

/*
Тест на добавление уже существующего файла с коллизией суффиксов
 */
test('add-file-with-exist-name-from-add-file-with-suffix-collision', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `a_${suffix}.png`, url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'a.png')).toBe(`a_${suffix}(1).png`);
});

/*
Тест на добавление уже существующего файла с разными суффиксами
 */
test('add-file-with-exist-name-from-add-file-with-suffix', () => {
    const newSuffix = '11111111';

    mockGenerateSuffix.mockReturnValueOnce(newSuffix);

    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: `a_22222222.png`, url: '25' },
    ];

    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'a.png')).toBe(`a_${newSuffix}.png`);
});
