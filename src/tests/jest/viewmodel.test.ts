...
Сценарий:
1. Новый файл вставленный через Ctr+V должен быть назван file_seg${segment_id}.png,
 */
...
вставки file_seg${segment_id}.png, file_seg${segment_id}(1).png, ... file_seg${segment_id}(4).png
2. Новый файл вставленный через Ctr+V должен быть назван также, но следующей
версией file_seg${segment_id}(5).png
 */
test('add-new-file-via-CtrV-test_3', () => {
    ...
    const files: LabkeeperFile[] = [
        ...
        {
            autogenerated: true,
            fileName: 'file_seg5(1).png',
            url: '25',
        },
        {
            autogenerated: true,
            fileName: 'file_seg5(2).png',
            url: '25',
        },
        {
            autogenerated: true,
            fileName: 'file_seg5(3).png',
            url: '25',
        },
        {
            autogenerated: true,
            fileName: 'file_seg5(4).png',
            url: '25',
        },
    ];
    repository.projectViewModelRepository.setFiles(files);
    ...
    expect(result).toBe('file_seg5(5).png');
});
...
вставки file_seg${segment_id}(i).png, но i это не {0, ... 3}
...
Пример: имеем file_seg${segment_id}.png, file_seg${segment_id}(1).png, file_seg${segment_id}(3).png,
file_seg${segment_id}(7).png
Вывод: file_seg${segment_id}(2).png
 */
test('add-new-file-via-CtrV-test_4', () => {
    ...
    expect(result).toBe('file_seg5(2).png');
});
 ...
 */
test('add-file_with_exist_name-with-dots-from-add-file-with-suffix', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: 'a.b.c(1).png', url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = null;
    expect(fileService.calculateNumberFile(segment_id, 'a.b.c.png')).toBe(
...
/*
Тест на добавление файла с уже существующим названием ((((1).png
 */
test('add-file_with_exist_name-with-brackets-from-add-file-with-suffix', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: '(((1).png', url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    const segment_id = null;
    expect(fileService.calculateNumberFile(segment_id, '(((.png')).toBe(
...
test('add-file-with-exist-name-from-add-file-with-suffixes-and-without-ext', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: 'abc(1)', url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'abc')).toBe('abc');
});
...
test('add-file-with-exist-name-from-add-file-with-suffix-collision', () => {
    const { repository, fileService } = mockContext();
    const files: LabkeeperFile[] = [
        { autogenerated: true, fileName: 'a(1).png', url: '25' },
    ];
    repository.projectViewModelRepository.setFiles(files);
    expect(fileService.calculateNumberFile(null, 'a.png')).toBe('a.png');
});
