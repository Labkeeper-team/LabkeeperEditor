...
export const fileUploadHeader = {"Content-Type": "multipart/form-data", "Accept": '*/*'};
...
export const checkFile = (file: File, dictionary: Translations) => {
    ...
    if(file.size > mbInBytes * maxSizeInMb) {
        toast(dictionary.filemanager.errors.tooBigFile.replace('${replace1}', maxSizeInMb.toString()), {type: 'error'});
