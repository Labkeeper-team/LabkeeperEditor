...
export const projectSlice = createSlice({
  ...
  reducers: {
    ...
    changeSegmentTextById: (
      ...
      const oldProgram = JSON.parse(JSON.stringify(state.history[state.historyAcitveIndex]));
      const newProgram = { ...oldProgram };
    ...
    changeSegmentPosition: (state, {payload}: PayloadAction<{
      ...
      const newProgram =  {...oldProgram};
      if (payload.direction === 'down' && newProgram.segments.length - 1 === payload.currentPosition) {
        return;
      }
      const currentSegment = {...newProgram.segments[payload.currentPosition]};
      const currentSegmentId = currentSegment.id;
      const changePositionIndex = payload.direction === 'up' ? payload.currentPosition - 1 : payload.currentPosition + 1;
      const changesSegment = {...newProgram.segments[changePositionIndex]};
      ...
      state.history = state.history.filter((_, index) => index <= state.historyAcitveIndex);
      state.history.push(newProgram);
      state.historyAcitveIndex = state.history.length - 1;
    },
    ...
    setSegmentVisibility: (
      state,
      { payload }: PayloadAction<{ index: number; visible: boolean, name: string }>
    ) => {
      const oldProgram = JSON.parse(
        JSON.stringify(state.history[state.historyAcitveIndex])
      );
      ...
      if (newProgram && newProgram.segments[payload.index]) {
        newProgram.segments[payload.index].parameters[payload.name] = payload.visible;
        state.history = state.history.filter((_, index) => {
          return index <= state.historyAcitveIndex;
        });
        state.history.push(newProgram);
    ...
    changeCompileErrorForSegment:(state, {payload: {segmentId, newErrors}}: PayloadAction<{segmentId: number, newErrors: CompileErrorResult[]}>) => {
      const oddErrors = state.compileErrorResult?.errors?.filter(err => err.payload.segmentId !== segmentId);
